...
class Detector:
    ...
    def __init__(self, detector_filename):
        ...
        if with_ros:
            ...
            if competition == "basketball":
                ...
                self.obstacles = rospy.Publisher('detectors/obstacle', Polygon, queue_size=1)
    ...
    if with_ros:
        def callback_basketball(self, image_msg):
            ...
            bbox, succes = detector.detect(frame, "ball detector")
            bbox_tl, bbox_br = bbox[0], bbox[1]
            #print(succes)
            #stages = detector.get_stages ("obstacle detector")
            #cv2.imshow ("frame", stages[2])
            ...
            if succes:
                ...
                #cv2.waitKey(2)
                #cv2.imshow ("ball", result)
                self.tennis_ball.publish(Point(float(x_b), float(y_b), float(0))) #hardcode before norm filters
            
            bbox, succes = detector.detect(frame, "basket detector")
           # stages = detector.get_stages("basket detector")
           # cv2.imshow("mask", stages[3])
            if succes:
                bbox_tl, bbox_br = bbox[0], bbox[1]
                x_b = (bbox_br[0] + bbox_tl[0])/2
                y_b = bbox_br[1]
                ...
                result = cv2.circle (frame.copy (), (int(x_b), int(y_b)), 5, (120, 150, 190), thickness = -1)
                #cv2.waitKey(2)
                #cv2.imshow ("basket", result)
                self.basket_bottom.publish(Point(float(x_b), float(y_b), float(0))) #hardcode before norm filters
